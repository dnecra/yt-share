...
const ALLOWED_FLAIRS = new Set(['🔥', '🔄️', '👀', '✨', '👍', '💤', '⁉️', '👎', '🚫', '🗑️']);
    ...
    try {
        ...
        for (const [videoId, entries] of Object.entries(data)) {
            ...
            for (const [ipKey, flair] of Object.entries(entries)) {
                ...
                if (!ALLOWED_FLAIRS.has(flair.emoji)) continue;
                flairMap.set(ipKey, {
                    emoji: flair.emoji,
                    ip: flair.ip || ipKey,
...
module.exports = {
    ALLOWED_FLAIRS: Array.from(ALLOWED_FLAIRS),
    getFlairsForVideo,
    setFlairForVideo,
