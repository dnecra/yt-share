...
function sanitizeLyricsCacheData(data) {
    ...
    delete copy.translatedPlainLyrics;
    delete copy.englishPlainLyrics;
...
function analyzeLyricsLanguage(data) {
    ...
    const eligibleLines = Array.isArray(syncedLines) && syncedLines.length > 0
        ? syncedLines
            .map((line) => (line?.text || '').toString().trim())
            .filter((text) => isEligibleTranslationLine(text))
            .filter(Boolean)
        : normalizeWrappedText(normalized?.plainLyrics || '')
            .split('\n')
            .map((line) => line.trim())
            .filter((text) => isEligibleTranslationLine(text))
            .filter(Boolean);
...
async function buildTranslatedLyricsData(data) {
    ...
    const plainText = normalizeWrappedText(normalized?.plainLyrics || '');
    if (!plainText) {
        return {};
    }

    const plainLines = plainText.split('\n');
    const eligiblePlainLines = plainLines.map((line, index) => ({ line, index }))
        .filter(({ line }) => shouldTranslateLyricLine(line));
    if (eligiblePlainLines.length === 0) {
        return {};
    }

    const translatedTexts = await translateLinesToEnglish(
        eligiblePlainLines.map(({ line }) => line)
    );

    const translatedPlainLines = [...plainLines];
    let translatedCount = 0;
    eligiblePlainLines.forEach(({ index, line }, translatedIndex) => {
        const translated = (translatedTexts[translatedIndex] || '').toString().trim();
        if (!translated || translated === line.trim()) return;
        translatedPlainLines[index] = translated;
        translatedCount += 1;
    });
    ...
    return { translatedPlainLyrics: translatedPlainLines.join('\n') };
}
...
async function resolveTranslationPayload(videoId, data) {
    ...
    if (cached && isTranslationPayload(cached.data)) {
        const cachedCount = Array.isArray(cached.data.translatedSyncedLyrics)
            ? cached.data.translatedSyncedLyrics.length
            : (typeof cached.data.translatedPlainLyrics === 'string' && cached.data.translatedPlainLyrics.trim()
                ? normalizeWrappedText(cached.data.translatedPlainLyrics).split('\n').length
                : 0);
        logEvent('info', 'LYRICS', `[TRANSLATION] Cache hit for ${videoId} (${cachedCount} lines)`);
        return cached.data;
    }
    ...
    const translatedCount = Array.isArray(payload?.translatedSyncedLyrics)
        ? payload.translatedSyncedLyrics.length
        : (typeof payload?.translatedPlainLyrics === 'string' && payload.translatedPlainLyrics.trim()
            ? normalizeWrappedText(payload.translatedPlainLyrics).split('\n').length
            : 0);
...
async function attachTranslation(videoId, data, options = {}) {
    ...
    const hasTranslatedSyncedLyrics = Array.isArray(payload?.translatedSyncedLyrics) && payload.translatedSyncedLyrics.length > 0;
    const hasTranslatedPlainLyrics = typeof payload?.translatedPlainLyrics === 'string' && payload.translatedPlainLyrics.trim().length > 0;
    if (!hasTranslatedSyncedLyrics && !hasTranslatedPlainLyrics) {
        return {
            ...data,
            translationLanguage,
    ...
    return {
        ...data,
        ...(hasTranslatedSyncedLyrics ? { translatedSyncedLyrics: payload.translatedSyncedLyrics } : {}),
        ...(hasTranslatedPlainLyrics ? { translatedPlainLyrics: payload.translatedPlainLyrics } : {}),
        translationLanguage,
...
async function attachCachedTranslation(videoId, data, options = {}) {
    ...
    const hasTranslatedSyncedLyrics = Array.isArray(cached.data.translatedSyncedLyrics) && cached.data.translatedSyncedLyrics.length > 0;
    const hasTranslatedPlainLyrics = typeof cached.data.translatedPlainLyrics === 'string' && cached.data.translatedPlainLyrics.trim().length > 0;
    if (!hasTranslatedSyncedLyrics && !hasTranslatedPlainLyrics) {
        return {
            ...data,
            translationLanguage,
    ...
    const translatedCount = hasTranslatedSyncedLyrics
        ? cached.data.translatedSyncedLyrics.length
        : normalizeWrappedText(cached.data.translatedPlainLyrics).split('\n').length;
    logEvent('info', 'LYRICS', `[TRANSLATION] Cache hit for ${videoId} (${translatedCount} lines)`);
    return {
        ...data,
        ...(hasTranslatedSyncedLyrics ? { translatedSyncedLyrics: cached.data.translatedSyncedLyrics } : {}),
        ...(hasTranslatedPlainLyrics ? { translatedPlainLyrics: cached.data.translatedPlainLyrics } : {}),
        translationLanguage,
        translationState: 'ready'
    };
}
...
function queueTranslationGeneration(videoId, data, baseResponseData, broadcast, meta = {}, options = {}) {
    ...
    const job = (async () => {
        try {
            ...
            const hasTranslatedSyncedLyrics = Array.isArray(translatedResponse?.translatedSyncedLyrics)
                && translatedResponse.translatedSyncedLyrics.length > 0;
            const hasTranslatedPlainLyrics = typeof translatedResponse?.translatedPlainLyrics === 'string'
                && translatedResponse.translatedPlainLyrics.trim().length > 0;
            if (!hasTranslatedSyncedLyrics && !hasTranslatedPlainLyrics) {
                return;
            }
