...
function hasRomanizationPayloadForLyricsData(payload, data) {
    if (!isRomanizationPayload(payload)) return false;

    const normalized = normalizeLyricsDataForCache(data);
    const hasSynced = !!(normalized?.syncedLyrics || normalized?.syncLyrics || normalized?.wordSyncedLyrics);
    if (hasSynced) {
        return !!payload.syncedLyrics;
    }

    if (typeof normalized?.plainLyrics === 'string' && normalized.plainLyrics.trim().length > 0) {
...
async function deleteRomanizationCacheEntry(videoId) {
    romanizationCache.delete(videoId);
    try {
        const filePath = path.join(LYRICS_PERSIST_DIR, `${videoId}${ROMANIZATION_CACHE_SUFFIX}`);
...
function hasTranslatedLyricsPayload(value) {
    return !!value && typeof value === 'object' && (
        (Array.isArray(value.translatedSyncedLyrics) && value.translatedSyncedLyrics.length > 0)
        || (typeof value.translatedPlainLyrics === 'string' && value.translatedPlainLyrics.trim().length > 0)
    );
}
...
async function resolveRomanizationPayload(videoId, data) {
    ...
    if (cached && hasRomanizationPayloadForLyricsData(cached.data, data)) {
        maybeUnloadRomanizationNow();
        return cached.data;
    }
    if (cached) {
        await deleteRomanizationCacheEntry(videoId);
        logEvent('info', 'LYRICS', `[ROMANIZATION] Regenerating stale romanization cache for ${videoId}`);
    }
...
function analyzeLyricsLanguage(data) {
    ...
    const sourceLines = Array.isArray(syncedLines) && syncedLines.length > 0
        ? syncedLines.map((line) => (line?.text || '').toString())
        : normalizeWrappedText(normalized?.plainLyrics || '').split('\n');
...
async function buildTranslatedLyricsData(data) {
    ...
    const eligiblePlainLines = plainLines
        .map((text, index) => ({ text, index }))
        .filter((line) => shouldTranslateLyricLine(line.text));
    if (eligiblePlainLines.length === 0) return {};

    const translatedTexts = await translateLinesToEnglish(
        eligiblePlainLines.map((line) => line.text)
    );
    const translatedPlainLines = new Array(plainLines.length).fill('');
    eligiblePlainLines.forEach((line, index) => {
        const translated = (translatedTexts[index] || '').toString().trim();
        if (!translated) return;
        translatedPlainLines[line.index] = translated;
    });

    if (!translatedPlainLines.some((line) => line.trim())) return {};
...
async function attachTranslation(videoId, data, options = {}) {
    ...
    if (!hasTranslatedLyricsPayload(payload)) {
        return {
            ...data,
            translationLanguage,
    ...
    return {
        ...
        translatedPlainLyrics: payload.translatedPlainLyrics,
        translationLanguage,
...
async function attachCachedTranslation(videoId, data, options = {}) {
    ...
    if (!hasTranslatedLyricsPayload(cached.data)) {
        return {
            ...data,
            translationLanguage,
            translationState: translationLanguage.isLikelyNonEnglish ? 'pending' : 'skipped'
    ...
    return {
        ...
        translatedPlainLyrics: cached.data.translatedPlainLyrics,
        translationLanguage,
        translationState: 'ready'
    };
}
...
function queueTranslationGeneration(videoId, data, baseResponseData, broadcast, meta = {}, options = {}) {
    ...
    const job = (async () => {
        try {
            ...
            if (!hasTranslatedLyricsPayload(translatedResponse)) {
                return;
            }
