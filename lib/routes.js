...
const { getApiKeys, hasUsableSecret } = require('./key-provider');
...
function normalizeNativeSearchItem(item, requestedType) {
    ...
    return {
        ...
        videoId,
        playlistId: videoId ? '' : findFirstValueByKey(renderer, 'playlistId'),
        browseId: videoId ? '' : findFirstValueByKey(renderer, 'browseId')
    };
}
...
async function fetchRapidApiCollectionItems(endpoint, id) {
    const { rapidApiHost, rapidApiKey } = getApiKeys();
    if (!hasUsableSecret(rapidApiKey)) {
    ...
    const response = await axios.get(`https://youtube-music-api3.p.rapidapi.com/${endpoint}`, {
    ...
    const flattenedItems = itemCandidates.find((candidate) => Array.isArray(candidate)) || [];
...
function setupRoutes(app, dependencies) {
    ...
    app.get('/api/v1/collection-items', async (c) => {
        try {
            ...
            const endpoint = kind === 'album' ? 'getAlbum' : 'getPlaylist';
            const items = await fetchRapidApiCollectionItems(endpoint, id);
            return c.json({
                success: true,
                items
            });
