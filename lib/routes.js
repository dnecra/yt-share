...
async function searchYouTubeMusicApi(query, type) {
...
function setupRoutes(app, dependencies) {
    ...
    app.get('/api/v1/search', async (c) => {
        try {
            ...
            const type = getSearchTypeOrDefault(c.req.query('type'));
            if (!query) {
                return c.json({ success: false, error: 'q is required' }, 400);
            }

            const data = await searchYouTubeMusicApi(query, type);
            return c.json({
                success: true,
                ...data
            });
