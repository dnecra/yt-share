...
async function pollForUpdates(isWebSocketConnected, fetchAllQueueItems, proxyRequest, broadcast, setQueueCache, getLastIsPaused, setLastIsPaused) {
    ...
    try {
        ...
        if (songResult.success && songResult.data) {
            const currentSongId = songResult.data.videoId;
            const currentIsPaused = songResult.data.isPaused ?? false;
            if (currentIsPaused !== getLastIsPaused()) {
...
async function pollProgress(isWebSocketConnected, proxyRequest, broadcast, getLastIsPaused, setLastIsPaused) {
    ...
    try {
        ...
        if (songResult.success && songResult.data) {
            const currentSongId = songResult.data.videoId;
            const currentIsPaused = songResult.data.isPaused ?? false;
            const currentProgress = songResult.data.elapsedSeconds;
            if (currentSongId && !currentIsPaused && currentProgress !== undefined) {
                broadcast({ 
                    type: 'song_progress', 
                    data: {
                        elapsedSeconds: currentProgress,
                        songDuration: songResult.data.songDuration || songResult.data.duration,
