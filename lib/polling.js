...
function buildQueueSignature(queueData) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        const selected = item?.selected === true || song?.selected === true ? '1' : '0';
 ...
 */
async function pollForUpdates(isWebSocketConnected, fetchAllQueueItems, proxyRequest, broadcast, setQueueCache, getLastIsPaused, setLastIsPaused, annotateQueueDataWithCurrentPlaying = null) {
...
function startPolling(dependencies) {
    ...
    setInterval(async () => {
        ...
        if (now - lastMainRun >= POLL_INTERVAL) {
            lastMainRun = now;
            await pollForUpdates(isWebSocketConnected, fetchAllQueueItems, proxyRequest, broadcast, setQueueCache, getLastIsPaused, setLastIsPaused, annotateQueueDataWithCurrentPlaying);
        }
