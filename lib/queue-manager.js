...
function getQueueItemKey(item, index = 0) {
    if (!item || typeof item !== 'object') {
    ...
    const setVideoId = renderer?.setVideoId || item.setVideoId || '';
    const playlistSetVideoId = renderer?.playlistSetVideoId || item.playlistSetVideoId || '';
    ...
    return [videoId, setVideoId, playlistSetVideoId, title, byline, lengthText].join('|');
}
...
async function fetchAllQueueItems(proxyRequest) {
    ...
    if (queueResult.success && queueResult.data) {
        ...
        const items = dedupeQueueItems(mergedItems);
        return {
            success: true,
            data: {
                items: items,
