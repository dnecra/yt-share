...
const { enrichQueueItemsWithFlairs } = require('./flair-manager');
const fs = require('fs');
...
const QUEUE_ATTRIBUTIONS_FILE_NAME = 'queue-attributions.json';

function resolveQueueAttributionsFile() {
    const candidates = [
        path.resolve(__dirname, '..', QUEUE_ATTRIBUTIONS_FILE_NAME),
        path.resolve(__dirname, QUEUE_ATTRIBUTIONS_FILE_NAME),
        path.resolve(process.cwd(), QUEUE_ATTRIBUTIONS_FILE_NAME)
    ];

    if (process.pkg) {
        candidates.unshift(path.resolve(path.dirname(process.execPath), QUEUE_ATTRIBUTIONS_FILE_NAME));
    }
    ...
    for (const filePath of candidates) {
        try {
            ...
            // Ignore inaccessible candidates and use the normal app location.
        }
    }
...
const QUEUE_ATTRIBUTIONS_FILE = resolveQueueAttributionsFile();

function loadQueueAttributionsFromDisk() {
    try {
        if (!fs.existsSync(QUEUE_ATTRIBUTIONS_FILE)) return;
        const data = JSON.parse(fs.readFileSync(QUEUE_ATTRIBUTIONS_FILE, 'utf8'));
        if (!data || typeof data !== 'object' || Array.isArray(data)) return;

        const now = Date.now();
        for (const [videoId, attribution] of Object.entries(data)) {
            if (!videoId || !attribution || typeof attribution !== 'object') continue;
            const timestamp = Number(attribution.timestamp);
            if (!attribution.ip || !timestamp || now - timestamp > VIDEO_ID_IP_MAP_TTL) continue;
            videoIdToIpMap.set(videoId, {
                ip: String(attribution.ip),
                timestamp,
                displayName: attribution.displayName ? String(attribution.displayName) : undefined
            });
        }
    } catch (error) {
        console.warn(`Could not load ${QUEUE_ATTRIBUTIONS_FILE_NAME}:`, error?.message || error);
    }
}

function saveQueueAttributionsToDisk() {
    try {
        const data = Object.fromEntries(videoIdToIpMap.entries());
        const tempPath = `${QUEUE_ATTRIBUTIONS_FILE}.tmp`;
        fs.writeFileSync(tempPath, JSON.stringify(data, null, 2), 'utf8');
        fs.renameSync(tempPath, QUEUE_ATTRIBUTIONS_FILE);
    } catch (error) {
        console.warn(`Could not save ${QUEUE_ATTRIBUTIONS_FILE_NAME}:`, error?.message || error);
    }
}
...
function getQueueItemSignature(item) {
    if (!item || typeof item !== 'object') {
        return 'invalid';
    }

    const renderer = getRenderer(item);
    const videoId = renderer?.videoId || extractVideoIdFromItem(item) || 'unknown';
    const title = renderer?.title?.runs?.map(run => run?.text || '').join('') ||
        renderer?.title?.simpleText ||
        item.title?.runs?.map(run => run?.text || '').join('') ||
        item.title?.simpleText ||
        '';
    const byline = renderer?.longBylineText?.runs?.map(run => run?.text || '').join('') ||
    ...
    return [videoId, title, byline, lengthText].join('|');
}

function getQueuePageSignature(items) {
    if (!Array.isArray(items)) {
        return '';
    }
    return items.map(getQueueItemSignature).join('\n');
}
...
async function enrichQueueItemsWithIpInfo(queueItems, getRandomNameForIp, normalizeIpAddress) {
    ...
    let removedExpiredAttribution = false;
    const enriched = queueItems.map((item, index) => {
        ...
        if (ipData && typeof ipData === 'object' && ipData.timestamp) {
            ...
            if (now - ipData.timestamp > VIDEO_ID_IP_MAP_TTL) {
                ...
                removedExpiredAttribution = true;
                return item;
            }
        }
        if (ipAddress) {
    ...
    if (removedExpiredAttribution) {
        saveQueueAttributionsToDisk();
    }
    const enrichedWithFlairs = enrichQueueItemsWithFlairs(enriched, extractVideoIdFromItem);
    if (enrichedCount > 0) {
        console.log(`[IP-ENRICH] Enriched ${enrichedCount} of ${totalItems} queue items with IP info`);
    }
    return enrichedWithFlairs;
}
...
async function fetchAllQueueItems(proxyRequest) {
    ...
    if (queueResult.success && queueResult.data) {
        const mergedItems = Array.isArray(queueResult.data.items) ? [...queueResult.data.items] : [];
        const seenTokens = new Set();
        const seenPageSignatures = new Set();
        if (mergedItems.length > 0) {
            seenPageSignatures.add(getQueuePageSignature(mergedItems));
        }
        let continuationToken = getContinuationToken(queueResult.data);
        ...
        while (continuationToken && !seenTokens.has(continuationToken)) {
            ...
            if (!nextPageResult.success || !nextPageResult.data) {
                break;
            }

            const nextItems = Array.isArray(nextPageResult.data.items) ? nextPageResult.data.items : [];
            if (nextItems.length === 0) {
                break;
            }

            const pageSignature = getQueuePageSignature(nextItems);
            if (seenPageSignatures.has(pageSignature)) {
                break;
            }
            seenPageSignatures.add(pageSignature);
            mergedItems.push(...nextItems);
            continuationToken = getContinuationToken(nextPageResult.data);
        }

        // Preserve every queue occurrence. The same video can be added multiple
        // times, and collapsing by videoId/title makes client indexes diverge
        // from the real YouTube queue.
...
async function broadcastQueueUpdate(broadcast, proxyRequest, enrichQueueItemsWithIpInfoFn, songAdded = false) {
    ...
    if (queueResult.success && queueResult.data) {
        ...
        try {
            await annotateQueueDataWithCurrentPlaying(queueResult.data, proxyRequest);
        } catch (e) {
...
function isQueueItemSelected(item) {
    const renderer = getRenderer(item);
    return item?.selected === true || renderer?.selected === true;
}

function clearCurrentPlayingMarkers(items) {
    if (!Array.isArray(items)) return;
    items.forEach(item => {
        if (!item || typeof item !== 'object') return;
        delete item._isCurrentlyPlaying;
        const renderer = getRenderer(item);
        if (renderer && typeof renderer === 'object') {
            delete renderer._isCurrentlyPlaying;
        }
    });
}

async function annotateQueueDataWithCurrentPlaying(queueData, proxyRequest) {
    if (!queueData || !Array.isArray(queueData.items)) {
        return queueData;
    }

    clearCurrentPlayingMarkers(queueData.items);
    delete queueData.currentlyPlayingIndex;

    const currentIndex = await getCurrentPlayingIndex(proxyRequest);
    if (currentIndex !== null && currentIndex >= 0 && currentIndex < queueData.items.length) {
        queueData.currentlyPlayingIndex = currentIndex;
        queueData.items[currentIndex]._isCurrentlyPlaying = true;
        const renderer = getRenderer(queueData.items[currentIndex]);
        if (renderer && typeof renderer === 'object') {
            renderer._isCurrentlyPlaying = true;
        }
    }
...
async function getCurrentPlayingIndex(proxyRequest) {
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            if (isQueueItemSelected(items[i])) {
                return i;
            }
        }
 ...
 */
async function navigateWithCommands(currentIndex, finalTargetIndex, proxyRequest, broadcast, enrichQueueItemsWithIpInfoFn, currentItem = null, targetItem = null) {
    ...
    for (let i = 0; i < absSteps; i++) {
        ...
        if (i % 3 === 2 || i === absSteps - 1) {
            ...
            if (verifiedIndex === finalTargetIndex) {
                ...
                return { success: true, method: 'sequential_commands', currentIndex, targetIndex: finalTargetIndex, currentItem, targetItem };
            }
...
async function navigateQueue(proxyRequest, broadcast, enrichQueueItemsWithIpInfoFn, targetIndex = null, relativeSteps = null) {
    ...
    try {
        ...
        const targetItem = queueResult.data.items[finalTargetIndex] || null;
        const currentItem = currentIndex !== null ? (queueResult.data.items[currentIndex] || null) : null;
        if (currentIndex !== null && currentIndex === finalTargetIndex) {
            return { success: true, method: 'noop', currentIndex, targetIndex: finalTargetIndex, currentItem, targetItem };
        }
        ...
        if (patchResult.success) {
            ...
            if (verifiedIndex === finalTargetIndex) {
                await broadcastQueueUpdate(broadcast, proxyRequest, enrichQueueItemsWithIpInfoFn);
                broadcast({ type: 'playback_updated' });
                return { success: true, method: 'http_patch', currentIndex: currentIndex ?? verifiedIndex, targetIndex: finalTargetIndex, currentItem, targetItem };
            }
            ...
            if (verifiedIndex !== null) {
                return await navigateWithCommands(verifiedIndex, finalTargetIndex, proxyRequest, broadcast, enrichQueueItemsWithIpInfoFn, queueResult.data.items[verifiedIndex] || null, targetItem);
            }
        ...
        if (workingIndex === finalTargetIndex) {
            await broadcastQueueUpdate(broadcast, proxyRequest, enrichQueueItemsWithIpInfoFn);
            broadcast({ type: 'playback_updated' });
            return { success: true, method: 'noop', currentIndex: workingIndex, targetIndex: finalTargetIndex, currentItem: queueResult.data.items[workingIndex] || null, targetItem };
        }
        return await navigateWithCommands(workingIndex, finalTargetIndex, proxyRequest, broadcast, enrichQueueItemsWithIpInfoFn, queueResult.data.items[workingIndex] || null, targetItem);
    } finally {
        isNavigating = false;
    }
}
