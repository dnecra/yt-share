...
const { handleExternalVolumeUpdate, extractVolume } = require('./volume-manager');
 ...
 */
async function handleWebSocketMessage(message, invalidateQueueCache, fetchAllQueueItems, enrichQueueItemsWithIpInfo, setQueueCache, annotateQueueDataWithCurrentPlaying = null) {
    try {
        if (message.type) {
            switch (message.type) {
                case 'queue_updated':
                case 'queue':
                    ...
                    if (message.data && message.data.items && Array.isArray(message.data.items)) {
                        ...
                        if (annotateQueueDataWithCurrentPlaying) {
                            await annotateQueueDataWithCurrentPlaying(queueData);
                        }
                    ...
                    if (queueResult.success && queueResult.data) {
                        ...
                        if (annotateQueueDataWithCurrentPlaying) {
                            await annotateQueueDataWithCurrentPlaying(queueResult.data);
                        }
                ...
                case 'volume':
                    {
                        const volume = extractVolume(message.data, message.volume);
                        if (volume !== null) {
                ...
                default:
                    if (message.queue !== undefined) {
                        ...
                        if (Array.isArray(message.queue)) {
                            const queueData = { items: await enrichQueueItemsWithIpInfo(message.queue) };
                            if (annotateQueueDataWithCurrentPlaying) {
                                await annotateQueueDataWithCurrentPlaying(queueData);
                            }
                            setQueueCache(queueData);
                    ...
                    if (message.volume !== undefined) {
                        const volume = extractVolume(message.volume);
                        if (volume !== null) {
                            handleExternalVolumeUpdate(volume, (v) => { lastVolume = v; }, broadcast);
                        }
                    }
